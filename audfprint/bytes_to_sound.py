...
prefix = '../debussy_2_sec/'
rs = RSCodec(10)

def process_byte(bytes_transfered, number):
    
    word = lookup[number]
    filename = f"{prefix}{word}"
    logging.info(f"#{bytes_transfered} : got {number} and playing {filename}")

    data, fs = soundfile.read(filename, dtype='float32')
    sounddevice.play(data, fs)#, device=args.device)
...
def main():
    ...
    logging.info(f'bytes to be processed: {len(encoded)}')
    for byte in encoded:
        process_byte(bytes_transfered, byte)
        bytes_transfered += 1
    logging.error("no more bytes")
